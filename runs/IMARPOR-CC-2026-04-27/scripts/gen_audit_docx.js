#!/usr/bin/env node
/**
 * gen_audit_docx.js — Emite DOCX FINAL para auditoría del instructor IMARPOR-CC.
 *   1. pm-3-6-FINAL-IMARPOR-CC.docx   — GFPI-F-135 Guía del Aprendiz (8 secciones + apéndices)
 *
 * Hito 5 scope HOY: SOLO PM-3.6 (Sergio aprobó A1).
 * PM-3.1 + PM-3.2 + PM-3.5 builds (presentes en código) NO se llaman desde main().
 * Adapt: subseccion auto-discovery (línea 2902) maneja s2-s9/s10/s11-s12 sin hardcode.
 *
 * Run: IMARPOR-CC-2026-04-27 · Programa Inglés Marítimo Portuario · Single-guía 12 sesiones · Canon v2.7
 */
...
const RUN_DIR = path.resolve(__dirname, '..');  // → runs/IMARPOR-CC-2026-04-27/  (portable · no session-id hardcode)
  ...
  default: new Footer({
    children: [new Paragraph({
      children: [
        ...
        new TextRun({ text: 'IMARPOR-CC · Inglés Marítimo Portuario · Canon v2.7  —  Página ', font: 'Calibri', size: 16, color: GREY }),
...
function buildPM31Docx() {
  ...
  return new Document({
    ...
    title: 'PM-3.1 Playbook Outline FINAL — IMARPOR-CC',
    description: 'Playbook Outline for final audit — IMARPOR-CC-2026-04-27',
    sections: [{
...
function buildPM32Docx() {
  ...
  return new Document({
    creator: 'FPI CD Engine v2.6',
    title: 'PM-3.2 Playbook Build-Out FINAL — IMARPOR-CC',
    description: '8 sesiones consolidadas para auditoría — IMARPOR-CC-2026-04-27',
    sections: [{
      properties: { page: { margin: { top: 1000, bottom: 1000, left: 1200, right: 1200 } } },
...
function buildPM36Docx() {
  ...
  return new Document({
    creator: 'FPI CD Engine v2.6',
    title: 'GFPI-F-135 Guía del Aprendiz FINAL — IMARPOR-CC',
    description: 'Guía del aprendiz para auditoría — IMARPOR-CC-2026-04-27',
    sections: [{
      properties: { page: { margin: { top: 1000, bottom: 1000, left: 1200, right: 1200 } } },
...
async function main() {
  // Hito 5 scope HOY: SOLO PM-3.6 (Sergio aprobó A1)
  // PM-3.1 + PM-3.2 + PM-3.5 quedan disponibles en código pero NO se invocan
  // (sus loops para 12 sesiones requieren adapt adicional · sesión futura)
  const outputs = [
    ['pm-3-6-FINAL-IMARPOR-CC.docx', buildPM36Docx],
  ];
