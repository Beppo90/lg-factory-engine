 ...
 *
 * Output: /sessions/focused-cool-dirac/mnt/fpi-sena-factory/runs/DIESEL-2026-04-19/
 */
...
function buildPM35Doc() {
  ...
  return new Document({
    ...
    description: 'DIESEL-2026-04-19 G1 The Workshop Assistant — Final Mission ABP',
    styles: {
...
function buildPM36Doc() {
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 240 },
    children: [new TextRun({ text: `Generado desde Playbook DIESEL-2026-04-19 · Tono: 2ª persona aprendiz · CEFR A1.1`, font: 'Calibri', size: 16, color: GREY })],
  }));
  ...
  // ===== Sección 4 — Planteamiento de Evidencias (formato SENA v2.6.4) =====
  ...
  const s4Children = renderSeccion4Evidencias(d, {
    ...
    helpers: {
      ...
      // La Sección 4 v2.6.4+ no invoca makeTable ni pageBreak en su rama principal, así que los shims vacíos son seguros.
      quote: (txt) => callout('📊 CANON DE PUNTUACIÓN', String(txt).replace(/^CANON DE PUNTUACIÓN\s*—\s*/, '')),
      note,
      makeTable: () => { throw new Error('makeTable no disponible en gen_35_36_docx.js — use schema v2.6.4+'); },
      pageBreak: () => P(''),
    },
  });
  for (const el of s4Children) children.push(el);
  ...
  return new Document({
    ...
    description: 'DIESEL-2026-04-19 G1 The Workshop Assistant — Learning Guide V02',
    ...
    sections: [{
      ...
      footers: standardFooter('GFPI-F-135 V02 — The Workshop Assistant'),
      children
    }]
  });
}
